...
public class OQLFilterVisitor extends OQLBaseVisitor<Filter> {
    ...
    protected Filter processEqExpr(EqualityExprContext ctx, String text0, String text1) {
        if (ctx.EQ() != null) {
    ...
    protected AbstractAtomicFilter processComparisonExp(ComparisonExprContext ctx, String text0, String text1) {
        Long value;
        try {
    ...
    @Override
    public Filter visitIsNotNullExpr(IsNotNullExprContext ctx) {
        String field = unescapeStringIfNecessary(ctx.expr().getText());
        return Filters.not(Filters.equals(field, (String) null));
    }
